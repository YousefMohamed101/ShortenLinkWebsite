// .wrangler/tmp/bundle-804C04/checked-fetch.js
...
// C:/Program Files (x86)/Nodist/bin/node_modules/wrangler/node_modules/@esbuild-plugins/node-globals-polyfill/process.js
...
// C:/Program Files (x86)/Nodist/bin/node_modules/wrangler/node_modules/@esbuild-plugins/node-globals-polyfill/Buffer.js
var lookup = [];
var revLookup = [];
...
var UrlEncoder_default = Generateshort;
...
app.get("/server/login/:username/:password", async (c) => {
  ...
  const supabase = getSupabase(c.env);
  const { data, error } = await supabase.from("Users").select("*").eq(loginColumn, username).eq("password", password).single();
  if (error) {
...
app.get("/server/GetLinkAnalysis/:LinkId", (c) => {
  ...
  const groupCount = (field) => data.reduce((acc, row) => {
...
app.get("/:shortcode", async (c) => {
  ...
  if (data) {
    const { error: error2 } = await supabase.from("ClickAnalytics").insert([{ link_id, ip_address: user_ip, country_code, user_agent, origin }]);
    console.log(link_id);
    console.log(data);
    console.log(error2);
...
app.post("/server/RegisterLink", async (c) => {
  ...
  const supabase = getSupabase(c.env);
  const { data, error } = await supabase.from("Links").insert([{ user_id: id, link_name: Name, Url }]).select().single();
  if (error) {
    return c.json({ error: error.message }, 500);
  }
  const shrtCode = UrlEncoder_default(data.id);
...
var SupabaseAdapter_default = app;

// C:/Program Files (x86)/Nodist/bin/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// C:/Program Files (x86)/Nodist/bin/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-804C04/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
  middleware_miniflare3_json_error_default
];
var middleware_insertion_facade_default = SupabaseAdapter_default;

// C:/Program Files (x86)/Nodist/bin/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-804C04/middleware-loader.entry.ts
